...
# Onready Variables
# ------------------------------------------------------------------------------
@onready var _audio1 : AudioStreamPlayer3D = $Audio1
@onready var _audio2 : AudioStreamPlayer3D = $Audio2
...
func _GetAvailableStream() -> AudioStreamPlayer3D:
	for stream in [_audio1, _audio2]:
		if not stream.playing:
			return stream
	return null

func _GetStreamName(audio : AudioStreamPlayer3D) -> StringName:
